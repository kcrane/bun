...
#elif CPU(S390X)
    return JSC::jsString(vm, makeAtomString("s390x"_s));
#else
...
#if CPU(BIG_ENDIAN)
    putDirectNamed(vm, variables, "node_byteorder"_s, JSC::jsString(vm, String("big"_s)));
#else
...
#elif CPU(S390X)
    putDirectNamed(vm, variables, "host_arch"_s, JSC::jsString(vm, String("s390x"_s)));
    putDirectNamed(vm, variables, "target_arch"_s, JSC::jsString(vm, String("s390x"_s)));
#else
